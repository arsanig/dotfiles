# Generated by Powerlevel10k configuration wizard on 2023-07-29 at 21:44 EDT.
...
# 12h time, angled separators, round heads, flat tails, 2 lines, disconnected, no frame,
...
() {
  ...
  typeset -g POWERLEVEL9K_USER_DEFAULT_BACKGROUND=132
  ...
  typeset -g POWERLEVEL9K_DIR_FOREGROUND=045
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local      clean='%048F'  # originally green foreground %76F
      local   modified='%208F'  # originally yellow foreground %178F
      local  untracked='%196F'  # orginially blue foreground %39F
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      ...
      (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${meta}${ahead_icon} ${VCS_STATUS_COMMITS_AHEAD}%f"
    ...
    # *42 if have stashes.
    # (( VCS_STATUS_STASHES        )) && res+=" ${meta}${stash_icon} ${VCS_STATUS_STASHES}"
    (( VCS_STATUS_STASHES        )) && res+=" ${meta}${stash_icon}"
    # 'merge' if the repo is in an unusual state.
    [[ -n $VCS_STATUS_ACTION     ]] && res+=" ${conflicted}${VCS_STATUS_ACTION}"
    # ~42 if have merge conflicts.
    # (( VCS_STATUS_NUM_CONFLICTED )) && res+=" ${conflicted}~${VCS_STATUS_NUM_CONFLICTED}"
    (( VCS_STATUS_NUM_CONFLICTED )) && res+=" ${conflicted}${conflict_icon}"
    # +42 if have staged changes.
    # (( VCS_STATUS_NUM_STAGED     )) && res+=" ${clean}${staged_icon} ${VCS_STATUS_NUM_STAGED}"
    (( VCS_STATUS_NUM_STAGED     )) && res+=" ${clean}${staged_icon}"
    # !42 if have unstaged changes.
    # (( VCS_STATUS_NUM_UNSTAGED   )) && res+=" ${untracked}${unstaged_icon} ${VCS_STATUS_NUM_UNSTAGED}"
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+=" ${untracked}${unstaged_icon}"
  ...
  typeset -g POWERLEVEL9K_DATE_BACKGROUND=254
  typeset -g POWERLEVEL9K_DATE_FOREGROUND='black'

  #################################[ Indentation fix RPROMPT ]#################################
